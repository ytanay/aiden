...
var Directory = module.exports = {
  ...
  join: function(callback){
    ...
    async.waterfall([
      ...
      (key, secondary, next) => {
        if(CONFIG.LIMITED_NETWORK) // If we are running on a limited network (i.e. computer lab simulation with no UPnP)
          return NetworkUtil.getLocalIPAddress(next); // Get the local IP address
        return next(null, '', '', next); // Unassign the error object, address, and hostname
      },
